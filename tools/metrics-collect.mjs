...
const categories = [
  ...
  { key: "finance", title: "Finance & Monetization", metrics: [
    m("active_subs", "Active subscriptions (incl. free trial)", num(sub.active), { formula: "status IN (active,trialing,authenticated) — includes free-trial users who have NOT paid", source: "DB subscriptions", owner: "Finance", priority: "H" }),
    m("paid_subs", "Paid subscribers (real money)", num(sub.paid_subs), { formula: "amount_paid>0 AND razorpay_payment_id present AND live users only — the true paying headcount", source: "DB subscriptions", owner: "Finance", priority: "H" }),
  ...
  { key: "app_store", title: "App Store (ASO)", metrics: [
    ...
    m("store_reviews", "iOS App Store reviews", "N/A — iOS skipped (owner decision 2026-07-01, i-20260701-71e5)", { formula: "owner marked App Store Connect N/A; Android reviews live via play_rating above", source: "owner directive", owner: "Support", priority: "L" }),
  ]},
  ...
  { key: "support", title: "Support", metrics: [
    ...
    m("csat_positive_pct", "CSAT positive rate (30d, ≥4★)", sup.csat_pct != null ? sup.csat_pct : (sup.csat_total_30d === 0 ? 0 : A("no 30d feedback yet")), { unit: sup.csat_pct != null ? `% (${sup.csat_total_30d || 0} responses, ≥4★)` : (sup.csat_total_30d === 0 ? "% (0 responses in window)" : ""), formula: "100*SUM(stars>=4)/COUNT(*) WHERE created_at>now()-30d", source: "DB app_feedback", owner: "Support", priority: "M" }),
    m("resolution_time", "Avg ticket resolution time (90d)", sup.avg_resolution_hrs != null ? sup.avg_resolution_hrs : (sup.resolved_tickets_90d === 0 ? "0 resolved tickets" : A("no resolved tickets yet")), { unit: sup.avg_resolution_hrs != null ? "hours" : "", formula: "avg(TIMESTAMPDIFF(HOUR,created_at,resolved_at)) WHERE resolved_at IS NOT NULL 90d window", source: "DB admin_support_tickets", owner: "Support", priority: "M" }),
  ]},
  ...
  { key: "competitors_legal", title: "Competitors & Legal", metrics: [
    ...
    m("competitor_healthifyme", "HealthifyMe est. MAU", comp ? (comp.competitors.find(c => c.name === "HealthifyMe") || {}).est_mau_k : null, { unit: "k users (est.)", formula: "manual quarterly estimate", source: "data/competitor_estimates.json", owner: "Strategy", priority: "L" }),
...
function appendHistory() {
  ...
  const snap = { date,
    ...
    active_subs: sub.active ?? null, paid_subs: sub.paid_subs ?? null,
    sub_revenue: sub.mrr_minor_units != null ? +(sub.mrr_minor_units / 100).toFixed(2) : null,
